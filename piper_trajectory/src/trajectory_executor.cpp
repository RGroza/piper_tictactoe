...
static const rclcpp::Logger LOGGER =
    rclcpp::get_logger("ttt_trajectory_executor");
...
TrajectoryExecutor::TrajectoryExecutor(rclcpp::Node::SharedPtr base_node)
...
void TrajectoryExecutor::loadWaypoints() {
  ...
  waypoints_.push_back(
      makePose(0.245f + GRID_X_OFFSET, 0.065f + GRID_Y_OFFSET, HOVERING_Z)); // 1
  waypoints_.push_back(
      makePose(0.200f + GRID_X_OFFSET, 0.065f + GRID_Y_OFFSET, HOVERING_Z)); // 2
  waypoints_.push_back(
      makePose(0.155f + GRID_X_OFFSET, 0.065f + GRID_Y_OFFSET, HOVERING_Z)); // 3
  waypoints_.push_back(
      makePose(0.245f + GRID_X_OFFSET, 0.020f + GRID_Y_OFFSET, HOVERING_Z)); // 4
  waypoints_.push_back(
      makePose(0.200f + GRID_X_OFFSET, 0.020f + GRID_Y_OFFSET, HOVERING_Z)); // 5
  waypoints_.push_back(
      makePose(0.155f + GRID_X_OFFSET, 0.020f + GRID_Y_OFFSET, HOVERING_Z)); // 6
  waypoints_.push_back(
      makePose(0.245f + GRID_X_OFFSET, -0.025f + GRID_Y_OFFSET, HOVERING_Z)); // 7
  waypoints_.push_back(
      makePose(0.200f + GRID_X_OFFSET, -0.025f + GRID_Y_OFFSET, HOVERING_Z)); // 8
  waypoints_.push_back(
      makePose(0.155f + GRID_X_OFFSET, -0.025f + GRID_Y_OFFSET, HOVERING_Z)); // 9
}
...
void TrajectoryExecutor::drawCircle(int cell_number) {
  ...
  double fraction = move_group_robot_->computeCartesianPath(
      circle_wps, end_effector_step_, jump_threshold_, traj);

  if (fraction < 0.9) {
    RCLCPP_WARN(LOGGER, "Circle trajectory planning failed (%.1f%%)",
                fraction * 100.0);
    return;
  }
...
void TrajectoryExecutor::moveToJointValue(float a0, float a1, float a2,
...
void TrajectoryExecutor::moveCartesian(float dx, float dy, float dz, float qx,
...
void TrajectoryExecutor::moveToClear() {
  // Sim
  // moveToJointValue(-0.470f, 1.463f, -0.867f, -0.000f, 1.062f, -0.470f);
  // Real
  Pose clear;
...
void TrajectoryExecutor::planAndExecuteKinematic() {
  if (move_group_robot_->plan(kinematics_trajectory_plan_) ==
